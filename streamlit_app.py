...
# sys.path.insert(0, os.getcwd())  # Make sure the current folder is included in imports

# # تأكد أن مسار المشروع موجود في sys.path
# project_dir = os.path.dirname(os.path.abspath(__file__))
# if project_dir not in sys.path:
#     sys.path.insert(0, project_dir)
# try:
#     import scraping
#     print("✅ Scraping module loaded!")
# except Exception as e:
#     print(f"❌ Failed to import scraping.py: {e}")
...
# Scraper wrapper
# ========================
def scraper_thread_func(email, password, pages, auto_bypass, log_queue):
    """Run scraper and send logs to the queue"""
    try:
        log_queue.put("🚀 Starting scraper...")
        log_queue.put(f"📧 Email: {email}")
        log_queue.put(f"🔐 Password: {'*'*len(password)}")
        log_queue.put(f"📄 Pages: {pages}")
        log_queue.put(f"🤖 Auto-bypass: {auto_bypass}")
        log_queue.put("-"*50)

        # Redirect stdout/stderr to capture logs
        buffer = io.StringIO()
        with contextlib.redirect_stdout(buffer), contextlib.redirect_stderr(buffer):
            scraping.run_scraper(email, password, pages)
        
        # Push captured output line by line
        for line in buffer.getvalue().splitlines():
            log_queue.put(line)
        
        log_queue.put("✅ Scraper completed successfully!")

    except Exception as e:
        log_queue.put(f"💥 Scraper error: {e}")
...
# Scraper runner
# ========================
def start_scraper(email, password, pages, auto_bypass):
    if st.session_state.scraper_running:
        st.warning("⚠️ Scraper is already running!")
        return
    st.session_state.scraper_running = True
    t = threading.Thread(
        target=scraper_thread_func,
        args=(email, password, pages, auto_bypass, st.session_state.log_queue),
        daemon=True
    )
    t.start()
...
logs = []
while st.session_state.scraper_running or not st.session_state.log_queue.empty():
...
# ========================
# Final logs if scraper finished
# ========================
while not st.session_state.log_queue.empty():
    logs.append(st.session_state.log_queue.get())
log_container.text("\n".join(logs))

st.success("✅ Scraper finished." if not st.session_state.scraper_running else "🔄 Running...")
